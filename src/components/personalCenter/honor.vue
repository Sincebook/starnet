  ...
  <div class="info-box">
    ...
    <div class="info notAllow" v-if="userinfo.user.status === 1">
      <el-alert
        ...
        :closable="false"
        center
        description="请前往实名认证页面进行认证或点击下方按钮进行跳转"
        show-icon
      >
      </el-alert>
      <div class="btn">
        ...
        >
          ...
          <el-form-item label="详情" prop="description">
            <el-input
              ...
              placeholder="请输入需要添加的工作经历"
            ></el-input>
          </el-form-item>
        </el-form>
      </div>
      <div class="title">
        <div class="name">已编辑工作经历</div>
      </div>
      <div class="info">
        <el-alert
          v-if="!isHave"
  ...
  methods: {
    ...
    submitForm() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          this.flag = true;
          addWorkInfo(this.ruleForm).then(res => {
            if (res.code === '0') {
              this.$message({
                message: '添加成功',
                type: 'success'
    ...
    getHonor() {
      getWorkInfo({ userid: this.userinfo.user.id }).then(res => {
        if (res.code === '0') {
          this.isHave = true;
          this.list = res.data;
        } else {
          this.isHave = false;
          this.$message.error('暂无经历');
        }
      }).catch(err => {
        this.isHave = false;
        return err;
